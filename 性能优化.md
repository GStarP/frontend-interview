...
- 协商缓存
  ...
  - Etag & If-None-Match
    ...
    - 优先级低于 Cache-Control，如果符合 Cache-Control 会直接从本地取缓存
    - 但刷新页面时浏览器会忽略缓存重新请求，这时结合使用 Etag 就能返回 304 减少开销
...
- 关键因素
  - 关键资源：网页首次渲染所需要的资源数
    ...
    - `<script defer>`：不阻塞渲染，后台下载解析；在页面渲染完毕后按序执行；执行完成后触发 DOMContentLoaded 事件
  - 关键路径：获取所有关键资源所需的时间
